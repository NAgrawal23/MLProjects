...
class ViewNormalizer:
    ...
    def normalize_identifier(identifier: str) -> str:
        ...
        # Handle escaped double quotes ""value""
    ...
    def normalize_table_name(table_name: str) -> str:
        """
        Normalize table name to just the base table name without catalog/schema
        
        Examples:
        - "catalog.schema.customers" -> "customers"
        - "schema.orders" -> "orders"
        - "customers" -> "customers"
        - ""catalog"".""schema"".""table"" -> "table"
        - '"production"."sales"."customers"' -> "customers"
        """
        if not table_name:
            return ""
        
        # First, normalize the identifier (remove quotes, lowercase)
        normalized = ViewNormalizer.normalize_identifier(table_name)
        
        # Skip if it's a SQL subquery
        if 'select' in normalized.lower() or normalized.startswith('('):
            return normalized
        
        # Handle cases where quotes are around each part: "catalog"."schema"."table"
        # After normalize_identifier, this might leave quotes in the middle
        # Clean up any remaining quotes around dots
        normalized = normalized.replace('"."', '.')
        normalized = normalized.replace("'.'", '.')
        normalized = normalized.replace('`.`', '.')
        normalized = normalized.strip('"').strip("'").strip('`')
        
        # Split by dots to handle catalog.schema.table
        parts = normalized.split('.')
        
        # Return only the last part (the actual table name)
        # catalog.schema.table -> table
        # schema.table -> table
        # table -> table
        base_table = parts[-1].strip().strip('"').strip("'").strip('`')
        
        return base_table if base_table else normalized
    ...
    def extract_tables(view_json: dict) -> Set[str]:
        ...
        for alias, table_ref in alias_map.items():
            try:
                if isinstance(table_ref, str):
                    # Use normalize_table_name to extract just the base table name
                    normalized = ViewNormalizer.normalize_table_name(table_ref)
                    if normalized:
                        tables.add(normalized)
                elif isinstance(table_ref, dict):
                    # Handle nested structures
                    if 'table' in table_ref:
                        normalized = ViewNormalizer.normalize_table_name(table_ref['table'])
                        if normalized:
                            tables.add(normalized)
                    elif 'name' in table_ref:
                        normalized = ViewNormalizer.normalize_table_name(table_ref['name'])
                        if normalized:
                            tables.add(normalized)
            except Exception:
        ...
        # Fallback: If no tables found in alias_map, try extracting from joins
        # This handles edge cases where alias_map might be missing
        if not tables:
            ...
            for join in joins:
                if isinstance(join, dict) and 'table' in join:
                    table_name = join['table']
                    if isinstance(table_name, str):
                        # Use normalize_table_name for consistency
                        normalized = ViewNormalizer.normalize_table_name(table_name)
                        if normalized:
                            tables.add(normalized)
...
class ViewSimilarityEngine:
    ...
    def compute_similarity_batch(self, view_id: int, candidate_ids: List[int], 
        ...
        # Bounds check on source view
        if view_id < 0 or view_id >= len(self.index.view_features):
            return []
        ...
        for candidate_id in candidate_ids:
            ...
            # Bounds check on candidate view
            if candidate_id < 0 or candidate_id >= len(self.index.view_features):
                continue
...
class ViewSimilarityFinder:
    ...
    def load_views_from_list(self, views_list: List[Dict], view_name_key: str = 'view_name',
        ...
        for idx, view_data in enumerate(views_list):
            ...
            try:
                ...
                if (idx + 1) % batch_size == 0:
                    ...
                    rate_str = f"{rate:.1f}" if rate != float('inf') else "(very fast)"
                    print(f"  Processed {idx + 1}/{len(views_list)} views "
                          f"({rate_str} views/sec, {error_count} errors)")
        ...
        if elapsed > 0:
            ...
            print(f"  Average rate: (extremely fast, <0.01 seconds)")
    ...
    def find_all_similarities(self, top_k: int = 5, 
        ...
        for view_id in range(n_views):
            ...
            if (view_id + 1) % batch_size == 0:
                ...
                if elapsed > 0:
                    ...
                    eta = (n_views - view_id - 1) / rate if rate > 0 else 0
                    print(f"  Processed {view_id + 1}/{n_views} views "
                          f"({rate:.1f} views/sec, ETA: {eta:.0f}s)")
                else:
                    print(f"  Processed {view_id + 1}/{n_views} views (very fast)")
        ...
        if elapsed > 0:
            ...
            print(f"  Average rate: (extremely fast, <0.01 seconds)")
        print(f"  Similar pairs found (before dedup): {len(all_results)}")
        
        # Convert to DataFrame
        df = self._format_results(all_results)
        
        # DEDUPLICATE: Remove reversed pairs (keep only source < similar_view)
        if not df.empty:
            # Create a sorted pair identifier for deduplication
            df['pair_key'] = df.apply(
                lambda row: tuple(sorted([row['source_view'], row['similar_view']])),
                axis=1
            )
            
            # Keep only first occurrence of each pair (unique combinations)
            df = df.drop_duplicates(subset=['pair_key'], keep='first')
            df = df.drop('pair_key', axis=1)
            
            print(f"  Similar pairs found (after dedup): {len(df)}")
